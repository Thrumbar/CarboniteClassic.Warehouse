...
local defaults = {
	profile = {
		Warehouse = {
			...
			AddTooltip = true,
			TooltipIgnore = true,
			IgnoreList = {["Hearthstone"]="Hearthstone"},			
			ShowGold = false,
		},
	},
}
...
function CarboniteWarehouse:OnInitialize()
	...
	local ttHooks = {
		"SetAction", "SetAuctionItem", "SetBagItem", "SetHyperlink", "SetInboxItem", "SetInventoryItem", "SetLootItem",
...
function Nx.Warehouse:CreateMenu()
	...
	local item = menu:AddItem (0, L["Remove Character"], self.Menu_OnRemoveChar, self)
...
function Nx.Warehouse:Update()
	...
	list:ItemAdd (99)
	list:ItemSetButton ("Warehouse", false, "Interface\\AddOns\\CarboniteClassic.Warehouse\\Gfx\\GroupNeedMore")
...
--	Nx.prt ("Find Link %s", link)
	...
	for cnum, rc in ipairs (Nx.RealmChars) do
		...
		local cnt = bagCnt + invCnt + bankCnt + mailCnt
